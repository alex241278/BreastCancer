...
def plot_axis_boxplots(
    ...
    label_weight = "bold" if bold_axis_labels else "normal"
    ...
    for ax_name in axes_order:
        ...
        _finalize_figure(
            fig,
            save_dir=save_dir,
            save_name=f"{save_prefix}_{safe_ax_name}.svg" if save_dir is not None else None,
            dpi=dpi,
        )
        plt.show()
...
def plot_axis_heatmap_by_subtype(
    ...
    cmap=None,
    save_dir=None,
    save_name="mean_axis_scores_by_subtype.svg",
    dpi=None,
    title_fontsize=None,
    tick_fontsize=None,
    annot_fontsize=None,
    ...
    ax.set_xticklabels(prettify_axis_list(axes_order), rotation=35, ha="right",
                       fontsize=tick_fontsize or PLOT_DEFAULTS["tick_fontsize"])
    ax.set_yticks(range(len(mat.index)))
    ax.set_yticklabels(mat.index, fontsize=tick_fontsize or PLOT_DEFAULTS["tick_fontsize"])
    if title:
        ax.set_title(title, fontsize=title_fontsize or PLOT_DEFAULTS["title_fontsize"])

    # anotaciones numéricas
    _afs = annot_fontsize or PLOT_DEFAULTS["annot_fontsize"]
    for i in range(mat.shape[0]):
        for j in range(mat.shape[1]):
            ax.text(j, i, f"{mat.iloc[i, j]:.2f}", ha="center", va="center", fontsize=_afs)

    divider = make_axes_locatable(ax)
    cax = divider.append_axes("right", size="4%", pad=0.10)
    cbar = fig.colorbar(im, cax=cax)
    cbar.set_label("Mean axis score",
    ...
    fig.tight_layout()
    _finalize_figure(fig, save_dir=save_dir, save_name=save_name if save_dir is not None else None, dpi=dpi)
    plt.show()
...
def plot_axis_correlation(
    ...
    cmap=None,
    save_dir=None,
    save_name="correlacion_spearman_ejes.svg",
    dpi=None,
    title_fontsize=None,
    tick_fontsize=None,
    annot_fontsize=None,
    ...
    for i in range(corr.shape[0]):
        for j in range(corr.shape[1]):
            ax.text(j, i, f"{corr.iloc[i, j]:.2f}", ha="center", va="center", fontsize=_afs)
...
def plot_axis_embedding(
    ...
    palette=None,
    save_dir=None,
    pca_save_name="pca_axes_space.svg",
    umap_save_name="umap_axes_space.svg",
    dpi=None,
    title_fontsize=None,
    axis_label_fontsize=None,
    tick_fontsize=None,
    ...
    _alfs = axis_label_fontsize or PLOT_DEFAULTS["axis_label_fontsize"]
    _tkfs = tick_fontsize or PLOT_DEFAULTS["tick_fontsize"]
    ...
    for cl in class_order:
        ...
        ax.scatter(
            ...
            alpha=0.8,
            s=_ss,
            color=palette_map[cl],
            edgecolors="none",
        )
    ax.set_xlabel("PC1", fontsize=_alfs)
    ax.set_ylabel("PC2", fontsize=_alfs)
    if pca_title:
        ax.set_title(pca_title, fontsize=_tfs)
    ax.tick_params(labelsize=_tkfs)
    ax.legend(frameon=False, fontsize=_lgfs)
    fig.tight_layout()
    _finalize_figure(fig, save_dir=save_dir, save_name=pca_save_name if save_dir is not None else None, dpi=dpi)
    plt.show()
    ...
    try:
        ...
        ax.set_xlabel("UMAP1", fontsize=_alfs)
        ax.set_ylabel("UMAP2", fontsize=_alfs)
        if umap_title:
            ax.set_title(umap_title, fontsize=_tfs)
        ax.tick_params(labelsize=_tkfs)
        ax.legend(frameon=False, fontsize=_lgfs)
        fig.tight_layout()
        _finalize_figure(fig, save_dir=save_dir, save_name=umap_save_name if save_dir is not None else None, dpi=dpi)
        plt.show()
...
def plot_one_vs_rest_heatmap(
    ...
    cmap=None,
    save_dir=None,
    save_name="one_vs_rest_heatmap.svg",
    dpi=None,
    ...
    ax.set_yticklabels(subtypes, fontsize=tick_fontsize or PLOT_DEFAULTS["tick_fontsize"])
    if title:
        ax.set_title(title, fontsize=title_fontsize or PLOT_DEFAULTS["title_fontsize"])
    ...
    for i in range(len(subtypes)):
        for j in range(len(axes)):
            ...
            if pd.isna(p):
                ...
                txt = ""
            if txt:
                ax.text(j, i, txt, ha="center", va="center", fontsize=_afs)
