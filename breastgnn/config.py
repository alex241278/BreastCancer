"""
Centralised configuration for BreastGNN.
...
---------------------
BREASTGNN_DATA_DIR          Directory containing expr_combat_corrected.csv and metadata_combined.csv
BREASTGNN_EXPR_CSV          Explicit expression CSV path
BREASTGNN_META_CSV          Explicit metadata CSV path
BREASTGNN_RAW_DATA_DIR      Directory where Zenodo files are downloaded
BREASTGNN_EXTERNAL_DATA_DIR Directory for OmniPath/HuRI raw files
BREASTGNN_CACHE_ROOT        Root directory for derived caches
BREASTGNN_ARTIFACTS_ROOT    Directory for trained artifacts
"""
...
DEFAULT_DATA_DIR = Path(os.environ.get("BREASTGNN_DATA_DIR", PROJECT_ROOT / "data" / "processed")).expanduser().resolve()
DEFAULT_RAW_DATA_DIR = Path(os.environ.get("BREASTGNN_RAW_DATA_DIR", PROJECT_ROOT / "data" / "raw")).expanduser().resolve()
DEFAULT_EXTERNAL_DATA_DIR = Path(os.environ.get("BREASTGNN_EXTERNAL_DATA_DIR", PROJECT_ROOT / "data" / "external")).expanduser().resolve()
DEFAULT_CACHE_ROOT = Path(os.environ.get("BREASTGNN_CACHE_ROOT", PROJECT_ROOT / "cache")).expanduser().resolve()
...
class BreastGNNConfig:
    ...
    def __post_init__(self):
        ...
        """
        self.DATA_DIR = Path(os.environ.get("BREASTGNN_DATA_DIR", self.DATA_DIR)).expanduser().resolve()
        self.RAW_DATA_DIR = Path(os.environ.get("BREASTGNN_RAW_DATA_DIR", self.RAW_DATA_DIR)).expanduser().resolve()
        self.EXTERNAL_DATA_DIR = Path(os.environ.get("BREASTGNN_EXTERNAL_DATA_DIR", self.EXTERNAL_DATA_DIR)).expanduser().resolve()
        self.CACHE_ROOT = Path(os.environ.get("BREASTGNN_CACHE_ROOT", self.CACHE_ROOT)).expanduser().resolve()

        expr_env = os.environ.get("BREASTGNN_EXPR_CSV")
        meta_env = os.environ.get("BREASTGNN_META_CSV")
        ...
        self.ARTIFACTS_ROOT = str(Path(os.environ.get("BREASTGNN_ARTIFACTS_ROOT", self.ARTIFACTS_ROOT)).expanduser().resolve())
